...
class TokenFlow(nn.Module):
    ...
    def batched_denoise_step(self, x, t, indices):
        ...
        # Compute sqrt mse between frames
        n_frames = len(x)
        mse = torch.zeros((n_frames, n_frames), device=x.device)
        for i in range(n_frames):
            for j in range(i + 1, n_frames):
                mse[i, j] = F.mse_loss(x[i], x[j], reduction='mean').sqrt()

        # Select pivotal_idx based on the largest differences
        # Create pairs of (i, j) where i < j
        pairs = [(i, j) for i in range(n_frames) for j in range(i + 1, n_frames)]
        # Sort pairs by mse in descending order
        pairs = sorted(pairs, key=lambda p: mse[p[0], p[1]], reverse=True)
        
        # Select the pairs with the largest mse values
        pivotal_pairs = pairs[:batch_size]
        pivotal_idx = torch.tensor([p[0] for p in pivotal_pairs] + [p[1] for p in pivotal_pairs]).unique()
        # print(pivotal_idx)
        # import pdb; pdb.set_trace()
        # Ensure the number of pivotal_idx does not exceed batch_size
        # if len(pivotal_idx) > batch_size:
        #     pivotal_idx = pivotal_idx[:batch_size]
...
if __name__ == '__main__':
    ...
    config["output_path"] = os.path.join(config["output_path"] + f'_pnp_SD_{config["sd_version"]}_ours_more',
