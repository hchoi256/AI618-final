...
class TokenFlow(nn.Module):
    ...
    def batched_denoise_step(self, x, t, indices):
        ...
        idx_division = list(range(0, len(x), batch_size)) # [0,8,16,24,32]
        idx_division.append(len(x)) # [0,8,16,24,32,40]
        pivotal_idx = [0] # 첫 번째 프레임은 무조건 포함
        # pivotal_idx = []
        # 첫 번째 프레임과의 MSE 계산
        for i in range(len(idx_division) - 1):
            # first_frame = x[idx_division[i]]
            first_frame = x[pivotal_idx[i]]
            mse_values = [F.mse_loss(first_frame, x[j]).sqrt() for j in range(idx_division[i] + 1, idx_division[i + 1])]
            # 가장 큰 MSE 값을 가지는 프레임의 인덱스를 추가
            max_mse_idx = torch.argmax(torch.tensor(mse_values)).item() + idx_division[i] + 1
            pivotal_idx.append(max_mse_idx)
