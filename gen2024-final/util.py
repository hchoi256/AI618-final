...
def save_video_frames_new(video_path, img_size=(512,512)):
    video_frames = os.listdir(video_path)
    # video_frames.sort()
    video_name = Path(video_path).stem
    os.makedirs(f'data_davis/{video_name}', exist_ok=True)
    # for i in range(len(video_frames)):
    #     ind = str(i).zfill(5)
        # image = T.ToPILImage()(video[i])
    for ind in video_frames:
        image = Image.open(os.path.join(video_path, ind))
        image_resized = image.resize((img_size),  resample=Image.Resampling.LANCZOS)
        import pdb; pdb.set_trace()
        image_resized.save(f'data_davis/{video_name}/{ind.split()[0]}.png')
        # image_resized.save(f'data/{video_name}/{ind}.png')
